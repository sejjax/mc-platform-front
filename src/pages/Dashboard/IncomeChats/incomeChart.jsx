...
const renderedLegend = (props, additionChartInfo) => {
  ...
  const financialProductIncome = payload.filter((el) => el.payload.key === 'fromFinancialProducts');
    ...
    <S.Wrapper className="income__legend_wrapper">
      <div>
        <S.LegendGrid mb className="legend__grid">
          <span>
            {t('dashboard_from_partners')} <S.Present>{partners.percent}%</S.Present>{' '}
          </span>
      ...
      <div>
        <S.LegendGrid mb className="legend__grid">
          <span>
            {t('dashboard_financial_product_total')}
        ...
        <PieChart width={200} height={250}>
          ...
          >
            {chartData.map((entry, index) => {
              return (
                <Cell className="some" key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
              );
            })}
