...
export const useIsProjectDisabled = (investorProLimit) => {
  const { projectCategory, investorLevel, baseDepositLevel, isDepositButtonDisabledByInvestment } =
  ...
  const checkInvestorPro = (project) => {
    ...
    return (
      isInvestorProLimitExceeded(investorProLimit) || !(investorLevel >= +project.investor_level)
    );
  };
  ...
  const isProjectDisabled = (project) => {
    ...
    if (projectCategory.slug === 'investor' || projectCategory.slug === 'investor-pro') {
      if (projectCategory.slug === 'investor-pro') return checkInvestorPro(project);
      const today = new Date();
      const startDate = new Date(project.startDate);
      const dueDate = new Date(project.dueDate);
      if (startDate > today || dueDate < today) {
    ...
    if (projectCategory.slug === 'bazovyj-paket') {
      const projectBaseLevel = parseInt(project.service_name.replace('basic', ''));
